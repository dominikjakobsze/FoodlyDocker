...
//get data from api point and fill forms with value attribute
export const getServerSideProps = async (context) => {
  ...
  const restaurant = await axios.get(
    "http://app-backend-foodly:8000/restaurant-by-token"
  );
  return {
    props: {
      access: access.data,
...
const update = ({ access, categories, cities, restaurant }) => {
  ...
  const handleSubmit = () => {
    ...
    axios
      .post("http://localhost:9600/restaurant-update", bodyFormData, {
        headers: {
  ...
  const [name, setName] = React.useState(restaurant.result[0].name);
  const [introduction, setIntroduction] = React.useState(
    restaurant.result[0].introduction
  );
  const [openclose, setOpenclose] = React.useState(
    restaurant.result[0].openclose
  );
  const [address, setaddress] = React.useState(restaurant.result[0].address);
