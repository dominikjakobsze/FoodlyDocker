...
const UpdateCategoryForm = ({ categoryId, categoryName }) => {
  const [content, setContent] = React.useState(categoryName);
  const { setContentModalMessage, setOpenModalMessage } =
    React.useContext(ModalMessageContext);
  const handleSubmit = () => {
    const bodyFormData = new FormData();
    bodyFormData.append("category_name", content);
    axios
      .post("http://localhost:9600/category/" + categoryId, bodyFormData, {
      ...
      .then((res) => {
        ...
        if (res?.data?.status === 403 || res?.data?.accepted === false) {
          ...
          setContentModalMessage((prev) => ({
            title: "Ok!",
            content: "Zmieniono nazwę kategorii",
          }));
          setOpenModalMessage(true);
        }
      });
