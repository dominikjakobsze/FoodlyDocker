...
const UpdateCityForm = ({ cityId, cityName }) => {
  const [content, setContent] = React.useState(cityName);
  ...
  const handleSubmit = () => {
    ...
    bodyFormData.append("city_name", content);
    bodyFormData.append("city_image", selectedFile);
    axios
      .post("http://localhost:9600/city/" + cityId, bodyFormData, {
        headers: {
      ...
      .then((res) => {
        ...
        if (res?.data?.status === 403 || res?.data?.accepted === false) {
          ...
          setContentModalMessage((prev) => ({
            title: "Ok!",
            content: "Zmieniono nazwę miasta i zdjęcie",
          }));
          setOpenModalMessage(true);
        }
      });
